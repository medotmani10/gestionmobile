    ...
    try {
      ...
      const mappedClients = clientsData.map((c: any) => {
        ...
        return {
          ...c,
          totalPaid: totalPaid,
          // Assuming 'total_debt' in DB is actually 'Total Invoiced Amount' or we calculate it from invoices
          // For now, let's assume the DB 'total_debt' column holds the 'Total Project Value/Invoiced'
          // And we calculate the actual remaining debt here.
          // IF 'total_debt' is updated by triggers, we use it. If not, let's assume it represents Total Invoiced.
          // Let's refine: Ideally, we sum up all 'invoices' for this client.
        };
      });
      ...
      const accurateClients = mappedClients.map(c => {
         ...
         // If DB total_debt is used for something else, let's overwrite for display
         // Total Debt displayed = Total Invoiced - Total Paid
         return {
           ...c,
           totalProjectsValue: totalInvoiced, // Helper prop
           totalDebt: totalInvoiced - c.totalPaid // Real remaining debt
         };
      });
          ...
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-6 animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleSavePayment} className="space-y-4">
              <div>
                ...
                <input type="number" required className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-lg font-bold text-slate-800" 
              ...
              <div>
                ...
                <textarea className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl" 
